// Romanian translations. Keep the same shape across every locale file.
// Romanian is still tentative — to drop it, remove this file and its entry in
// src/i18n/index.ts plus the { code: 'ro' } item in LanguageSwitcher.tsx.
// Brand = Emoție · Cogniție · Acțiune -> ECA.
const ro = {
  brand: 'ECA',
  nav: {
    project: 'Proiectul ECA',
    partners: 'Parteneri',
    resources: 'Resurse',
    experience: 'Impresii',
  },
  home: {
    title: 'Bine ai venit la ECA',
    subtitle: 'Un site de prezentare multilingv — aceasta este o pagină de start temporară.',
    cta: 'Descoperă proiectul',
  },
  project: {
    title: 'Proiectul ECA',
    paragraphs: [
      'Programul ECA (Emoție–Cogniție–Acțiune) este o inițiativă pedagogică inovatoare. Fundamentul său intelectual este pedagogia centrată pe persoană, care se bazează pe unicitatea elevilor, pe motivația lor interioară și pe siguranța emoțională.',
      'Acronimul ECA reprezintă unitatea dintre Emoție, Cogniție și Acțiune. Acest model subliniază că procesul de învățare devine complet atunci când elevii se conectează emoțional cu materia, o prelucrează cognitiv și apoi își aplică cunoștințele prin acțiune activă. Scopul programului este ca cei mici nu doar să dobândească cunoștințe, ci și să se bucure de procesul de învățare și să își poată lega studiile de propria viață.',
      'Un obiectiv prioritar al nostru este să dezvoltăm o practică pedagogică ce poate oferi un model pentru învățământul public. ECA oferă o viziune adaptabilă și un set de instrumente metodologice care pot fi integrate și în mediul școlar tradițional. În dezvoltarea programului colaborăm cu organizația poloneză Gedania1922, partenerul nostru în această activitate în cadrul proiectului Erasmus+.',
      'Grupul-țintă al programului este format din copii din clasele 1–4 și din cadrele didactice. Este deosebit de important pentru noi să asigurăm un mediu de învățare potrivit și pentru copiii cu un nivel de activitate ridicat, greu de reglat. Programul se bazează în mod conștient pe particularitățile de vârstă, întrucât nevoile fundamentale ale școlarilor mici sunt mișcarea, jocul, explorarea și conectarea socială.',
      'Pe parcursul programului ECA iau naștere și produse concrete: un curriculum bazat pe logica ECA, un manual practic și un curriculum de formare continuă pentru cadrele didactice. În ansamblu, scopul programului ECA este crearea unui mediu de învățare în care copiii participă activ, cu bucurie și în mod personal, oferind în același timp cadrelor didactice instrumente noi, aplicabile flexibil, pentru munca de zi cu zi.',
    ],
  },
  partners: {
    title: 'Parteneri',
    // NOTĂ: traducere generată automat din maghiară — de verificat înainte de publicare.
    paragraphs: [
      'Gedania 1922 este unul dintre cele mai vechi cluburi sportive din Polonia, cu peste 800 de membri. Din structura sa fac parte o grădiniță pentru 150 de copii, cu două colțuri de joacă, o școală alternativă cu 80 de elevi și o școală sportivă de 100 de elevi. Pe lângă formarea fotbaliștilor și a antrenorilor, scopul principal al clubului este îmbinarea educației inovatoare bazate pe competențe socio-emoționale cu activitățile sportive. Programul educațional al școlii și grădiniței alternative creoGedania leagă dezvoltarea inteligenței emoționale de introducerea instrumentelor TIC (tehnologii ale informației și comunicării) în educația școlară și preșcolară. Clubul Sportiv Gedania are peste 120 de angajați și este vizitat frecvent atât de elevi, cât și de oaspeți din străinătate. Gedania 1922 își propune să creeze un nou spațiu educațional în care ideile creative ale copiilor mici sunt luate în serios și din punct de vedere științific. Suntem, de asemenea, parteneri ai Polish Academy of Kids, care derulează programul Young Scientists. Care sunt activitățile organizației în domeniul acestui proiect? Gedania 1922 pregătește copiii pentru a-și folosi inteligența emoțională (EQ), le lărgește orizontul intelectual cu ajutorul unor instrumente TIC alese în mod conștient și îi ajută să privească știința în mod natural, ca pe un joc minunat. Echipa noastră este înalt calificată în educația bazată pe jocul liber și învață continuu prin experiența de zi cu zi cu copiii. Am participat la proiecte de finanțare de amploare mai mică, locale și naționale, legate de învățarea socială și emoțională (SEL) și de competențele viitorului prezente în educația cotidiană (autonomie, reziliență, empatie, cooperare etc.).',
      'Fundația Meseliget este o organizație a societății civile în plină dezvoltare, centrată pe familie, dedicată dezvoltării armonioase a copiilor și construirii unor rețele comunitare de sprijin și acceptare. Pe lângă activitățile variate de consiliere și mentorat, în structura sa se integrează strâns și cea mai recentă inițiativă, Rogers Jurtasuli (Școala-iurtă), care oferă copiilor un mediu de învățare alternativ, apropiat de natură. Pe lângă sprijinul continuu acordat familiilor și cadrelor didactice, scopul principal al fundației este îmbinarea educației inovatoare — bazate pe competențe socio-emoționale și pe o abordare restaurativă — cu practica pedagogică de zi cu zi. Programul educațional și de mentorat al Fundației Meseliget leagă în mod organic dezvoltarea țintită a inteligenței emoționale (EQ) de utilizarea conștientă și interactivă a instrumentelor TIC moderne (tehnologii ale informației și comunicării) în școală și grădiniță. Echipa profesională a fundației este formată din specialiști înalt calificați — printre care pedagogi cu experiență, traineri centrați pe persoană, mentori ADHD și mediatori —, care colaborează constant cu comunitățile locale, cu părinții și cu instituțiile de învățământ. Aspirația fundamentală a organizației este crearea unui spațiu educațional nou și deschis, în care nevoile individuale, ideile creative și particularitățile atipice de dezvoltare ale copiilor sunt pe deplin recunoscute și luate în serios din punct de vedere profesional și științific.',
      'Prin programele sale cuprinzătoare, Fundația Meseliget pregătește în mod conștient copiii pentru utilizarea zilnică a inteligenței emoționale (EQ), lărgindu-le totodată în mod țintit orizontul intelectual și cognitiv cu ajutorul unor instrumente digitale și TIC atent alese, bazate pe experiență. Un pilon al filosofiei lor educaționale este ca cei mici să trăiască învățarea și științele nu ca pe o constrângere, ci în mod natural, ca pe o experiență și un joc. Echipa profesională a organizației are o experiență și o pregătire remarcabile în educația bazată pe jocul liber, pe pedagogia de proiect și pe tehnicile cooperative; își dezvoltă continuu cunoștințele prin experiența practică zilnică dobândită cu copiii și cu familiile cu cerințe educaționale speciale (CES) și ADHD.',
      'Fundația participă în mod regulat și cu succes la diverse proiecte de finanțare la nivel local și național. Aceste inițiative se concentrează în principal pe răspândirea metodologiei învățării sociale și emoționale (SEL), pe pedagogia jocurilor de societate și pe integrarea competențelor-cheie ale viitorului în educația de zi cu zi — cu o atenție deosebită acordată autonomiei personale, rezilienței, empatiei, gestionării restaurative a conflictelor și cooperării eficiente în grup.',
    ],
  },
  resources: {
    title: 'Resurse',
    // NOTĂ: traducere generată automat din maghiară — de verificat înainte de publicare.
    paragraphs: [
      'Dacă ar fi să rezumăm scopul proiectului într-o singură frază, aceasta ar suna astfel: să pregătim învățătorii pentru a putea oferi elevilor cea mai mare autonomie de învățare posibilă, într-un climat de învățare cât mai sigur cu putință. Acesta este și scopul final al curriculumului nostru.',
      'Manualul cuprinde instrumente practice pentru o educație centrată pe persoană, bazată pe ECA (Emoție–Cogniție–Acțiune), la ciclul primar. Ca învățător, vei găsi aici jocuri, șabloane, schițe de lecție și liste de verificare. Pe măsură ce te familiarizezi cu aceste instrumente practice, îți însușești și atitudinea pedagogiei centrate pe persoană, întrucât ECA se bazează pe aceasta.',
    ],
    items: [
      {
        title: 'ECA QUICK MAP',
        body: 'O busolă vizuală, ușor de parcurs, pentru întregul program ECA.',
      },
      {
        title: 'Model de lecție ECA',
        body: 'Un șablon ușor de utilizat.',
      },
    ],
    handbook: 'Manualul profesorului ECA',
    download: 'Descarcă (PDF)',
  },
  experience: {
    title: 'Impresii',
    // NOTĂ: traducere generată automat din maghiară — de verificat înainte de publicare.
    headline: 'Când învățarea nu ține doar de minte – un workshop ECA la Jászberény',
    lead: [
      'Ce anume face ca o experiență de învățare să fie cu adevărat durabilă? Faptul că primim multe informații? Sau faptul că ceva ne atinge pe parcurs?',
      'La un workshop universitar din Jászberény am căutat cum poate fi învățarea, în același timp, experiență, gândire și acțiune.',
    ],
    caption: 'La Jászberény am prezentat ECA',
    gallery: {
      title: 'Galerie',
      alt: 'Fotografie de la workshopul din Jászberény',
      close: 'Închide',
      prev: 'Înapoi',
      next: 'Înainte',
    },
    sections: [
      {
        heading: 'O dimineață care nu a fost o „oră”',
        paragraphs: [
          'La Campusul din Jászberény al Universității Catolice Eszterházy Károly, 33 de studenți la învățământ cu frecvență redusă și 5 elevi de liceu au participat la o activitate de trei ore. Pentru cei mai mici a fost o activitate opțională, iar pentru studenți s-a integrat în orele obligatorii. Scopul nu a fost transmiterea unei noi materii, ci crearea unui cu totul alt fel de experiență de învățare. Ne-am străduit ca participanții să nu fie ascultători pasivi, ci artizani activi ai procesului.',
        ],
      },
      {
        heading: 'Ce este ECA?',
        paragraphs: ['ECA înseamnă trei lucruri simple, dar fundamentale:'],
        list: [
          'Emoție – cum mă simt, dacă sunt în siguranță',
          'Cogniție – ce înțeleg, cum gândesc',
          'Acțiune – ce încerc, ce fac',
        ],
        after: ['Când aceste trei elemente sunt în echilibru, învățarea începe cu adevărat să funcționeze.'],
      },
      {
        heading: 'Nu am vorbit despre ea – am făcut-o!',
        paragraphs: ['În cursul dimineții, participanții:'],
        list: [
          's-au conectat unii cu alții prin jocuri',
          'au reflectat asupra învățării pornind de la propriile experiențe',
          'au experimentat colaborarea în sarcini creative',
          'și-au proiectat propriile programe',
          'au gândit împreună și au reflectat asupra sensului întregului proces',
        ],
        after: [
          'A fost un joc al statuilor, creație comună, exercițiu de situație, brainstorming – și, între timp, se năștea mereu o realizare importantă: nu poți învăța fără implicare.',
          'Una dintre întrebările-cheie ale workshopului a fost aceasta: „Unde sunt eu în procesul de învățare?” Ca învățător, ca părinte, ca persoană care sprijină – cât de prezent sunt? Sunt atent la celălalt? Îi ofer spațiu?',
        ],
      },
      {
        heading: 'Ce au dus acasă?',
        paragraphs: [
          'La finalul zilei, participanții au dat titluri experienței lor și au formulat ce le-ar povesti altora despre ea. Au apărut răspunsuri foarte diferite, dar un lucru a fost comun: această dimineață nu s-a adresat doar minții lor, ci întregii lor ființe – deopotrivă emoțiilor, corpului și intelectului. Și poate că tocmai aceasta este adevărata esență a învățării.',
        ],
      },
    ],
  },
  footer: {
    rights: 'Toate drepturile rezervate.',
  },
} as const

export default ro
